...
class MagnitudeProvider extends ChangeNotifier {
  ...
  changeValues() {
    motionSensors.magnetometer.listen((MagnetometerEvent event) {
      ...
      x = magnetometer.x; // Set values to variables
      y = magnetometer.y;
      z = magnetometer.z;
      magnitude = sqrt((pow(magnetometer.x, 2)) +
          (pow(magnetometer.y, 2)) +
          (pow(magnetometer.z, 2)));
      values.add(LiveData(x, y, z, time++)); // Adding data in list
      if (values.length > 40) {
        values.removeAt(0);
      }
